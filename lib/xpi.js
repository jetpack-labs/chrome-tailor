...
function xpi(options) {
  ...
  return new Promise(function(resolve) {
    ...
    //mkdir.sync(crxPath);
    copy(process.cwd(), crxPath);
    console.log("Copied chrome extension");

    // create a package.json
    console.log("Creating package.json...");
    var chromeManifest = require(path.join(cwd, "manifest.json"));
    var packageJSON = convertManifest(chromeManifest);
    fs.writeFile(path.join(tmpDir, "package.json"), JSON.stringify(packageJSON) + "\n");
    ...
    then(function(xpiPath) {
      copy(xpiPath, path.join(process.cwd(), xpiName));
    }).
    */
    then(function(xpiPath) {
      process.chdir(cwd);
